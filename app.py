...
def get_global_wind_sites(center_lat, center_lon):
    import random
    import math
    
    # 1. 25 Dynamic organic local points based on current `Search Location`
    # We use a deterministic random seed so the points don't jump around on every single refresh
    r_gen = random.Random(f"{center_lat:.1f},{center_lon:.1f}")
    
    l_coords = []
    for _ in range(25):
        # Generate points within a ~300km radius (approx 0.3 to 3.0 degrees)
        r = r_gen.uniform(0.3, 3.0)
        theta = r_gen.uniform(0, 2 * math.pi)
        p_lat = center_lat + r * math.cos(theta)
        p_lon = center_lon + r * math.sin(theta)
        l_coords.append((round(p_lat, 4), round(p_lon, 4)))
        
    # 2. Key Global & Indian Wind Hubs to ensure world-wide & local representation
    g_coords = [
        # Global
        (54.0, 3.0), (-45.0, -68.0), (35.0, -100.0), (-30.0, 115.0), 
        (40.0, 140.0), (10.0, -75.0), (-20.0, 45.0), (55.0, -120.0), 
        # India (Muppandal, Kutch, Karnataka, Jaisalmer, Offshore Mumbai)
        (8.2, 77.5), (23.5, 68.5), (15.3, 75.6), (26.9, 70.9), (19.0, 72.0) 
    ]
    
    grid_lats = [lat for lat, lon in g_coords]
    grid_lons = [lon for lat, lon in g_coords]
    
    for lat, lon in l_coords:
        grid_lats.append(lat)
        grid_lons.append(lon)
            
    # Process in one chunk (approx 38 points total < 50)
    chunk_size = 50
    sites = []
    
    for i in range(0, len(grid_lats), chunk_size):
        c_lats = grid_lats[i:i+chunk_size]
        c_lons = grid_lons[i:i+chunk_size]
        
        url_current = "https://api.open-meteo.com/v1/forecast"
        params_c = {
            "latitude": ",".join(map(str, c_lats)),
            "longitude": ",".join(map(str, c_lons)),
        ...
        try:
            # Historical data fetching is intensive, allow up to 40 seconds timeout
            res_c = requests.get(url_current, params=params_c, timeout=20)
            res_h = requests.get(url_hist, params=params_h, timeout=40)
            
            data_c = res_c.json() if res_c.status_code == 200 else []
            data_h = res_h.json() if res_h.status_code == 200 else []
            
            if isinstance(data_c, list) and isinstance(data_h, list):
                for idx in range(len(c_lats)):
                    loc_c = data_c[idx] if idx < len(data_c) else {}
                    loc_h = data_h[idx] if idx < len(data_h) else {}
                    
                    current = loc_c.get("current", {})
                    ws10 = current.get("wind_speed_10m", 0)
                    ws80 = current.get("wind_speed_80m", 0)
                    
                    daily_h = loc_h.get("daily", {})
                    wind_arr = daily_h.get("wind_speed_10m_max", [])
                    wind_arr = [w for w in wind_arr if w is not None]
                    hist_avg = round(sum(wind_arr) / len(wind_arr), 2) if wind_arr else 0
                    
                    # Only map sites with realistic wind potential globally to avoid clutter
                    if ws80 >= 12.0 or hist_avg >= 12.0:
                        sites.append({
                            "lat": c_lats[idx],
                            "lon": c_lons[idx],
                            "ws10": ws10,
...
if mode == "Manual":
    ...
    **✅ How to mark your Roof/Area:**
    ...
    heat_data = get_global_solar_heatmap()
    ...
    # Fetch real-time wind data for global and dynamic local grid
    wind_sites = folium.FeatureGroup(name='🌪️ Global Wind Potential Sites', show=False)
    regional_wind_data = get_global_wind_sites(DEFAULT_LAT, DEFAULT_LON)
    
    if regional_wind_data:
        # Show all sites natively based on historical 5-year and real-time viability
        for site in regional_wind_data:
            ws80 = site['ws80']
            ws10 = site['ws10']
            hist_avg = site.get('hist_avg', 0)
            
            # Combine real-time and 5-yr historical data for status
            if ws80 >= 25.0 or hist_avg >= 25.0:
                color = "darkgreen"
                status = "Excellent Potential"
            elif ws80 >= 18.0 or hist_avg >= 18.0:
                color = "green"
                status = "Good Potential"
            elif ws80 >= 12.0 or hist_avg >= 12.0:
                color = "orange"
                status = "Moderate Potential"
            else:
                color = "gray"
                status = "Sub-optimal Potential"
                
            folium.Marker(
                [site['lat'], site['lon']], 
                icon=folium.Icon(color=color, icon="cloud"),
                popup=f"<b>Wind Potential: {status}</b><br>5-Yr Hist Avg (10m): {hist_avg} km/h<br>Live 80m Wind: {ws80} km/h<br>Live 10m Wind: {ws10} km/h"
            ).add_to(wind_sites)
    else:
        # Fallback if API fails
        fallback_r = random.Random("wind_fallback")
        for _ in range(3):
            w_lat = fallback_r.uniform(-60.0, 75.0)
            w_lon = fallback_r.uniform(-180.0, 180.0)
            folium.Marker(
                [w_lat, w_lon], 
    ...
    # Render map
    st_data = st_folium(m, width="stretch", height=500, returned_objects=["last_active_drawing", "last_clicked"], key="solar_map")
...
if size_choice == "Custom Capacity (kW)":
    # Using new 22% efficiency: 1 kW requires approx 4.54 sq.m
    default_cap = round(area * 0.22, 1) if area >= 1.0 else 1.0
    capacity_kw = st.sidebar.number_input("System Capacity (kW)", value=float(default_cap), min_value=0.1, step=0.5)
    area = capacity_kw / 0.22
else:
...
weather = get_live_solar_data(round(lat, 4), round(lon, 4))
if weather:
    ...
    ghi = weather.get("ghi", "--")
    st.info(f"⛅ **Live Weather** — Temp: {temp}°C | Wind: {wind} km/h ({wind_ms:.1f} m/s) | Status: {w_desc} | Irradiance: {ghi} W/m² 🌞", icon="⛅")
...
with tab1:
    ...
    ds_values = [
        ...
        f"{yearly_energy:.2f}", f"{ml_daily_energy:.2f}", f"{ml_daily_energy * 30.41:.2f}", 
        f"{ml_daily_energy * 365.25:.2f}", f"{total_cost:,.0f}", f"{monthly_sav:,.2f}", 
